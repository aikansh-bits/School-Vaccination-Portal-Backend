...
export const createDrive = async (req, res) => {
    try {
        const { vaccineName, scheduledDate, dosesAvailable, applicableClasses, createdBy } = req.body;

        if (!vaccineName || !scheduledDate || !dosesAvailable || !applicableClasses || !createdBy) {
        ...
        if (diffDays < 15) {
            return res.status(400).json({ status: "error", message: "Scheduled date must be at least 15 days from today" });
        }
...
export const updateDrive = async (req, res) => {
    try {
        ...
        if (new Date(drive.scheduledDate) < new Date()) {
            return res.status(400).json({ status: "error", message: "Cannot edit a past drive." });
        }

        const updatedDrive = await Drive.findByIdAndUpdate(driveId, updates, { new: true });
